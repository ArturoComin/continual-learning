...
def define_args(filename, description):
    parser = argparse.ArgumentParser(
        "./{}.py".format(filename), description=description
    )
    return parser


def add_general_options(
    parser, main=False, comparison=False, compare_hyper=False, pretrain=False, **kwargs
):
    if main:
        parser.add_argument(
            "--get-stamp", action="store_true", help="print param-stamp & exit"
        )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="[first] random seed (for each random-module used)",
    )
    if comparison and (not compare_hyper):
        parser.add_argument(
            "--n-seeds", type=int, default=1, help="how often to repeat?"
        )
    parser.add_argument(
        "--no-gpus", action="store_false", dest="gpu", help="don't use GPUs"
    )
    parser.add_argument(
        "--no-save", action="store_false", dest="save", help="don't save trained models"
    )
    parser.add_argument(
        "--full-stag",
        type=str,
        metavar="STAG",
        default="none",
        help="tag for saving full model",
    )
    parser.add_argument(
        "--full-ltag",
        type=str,
        metavar="LTAG",
        default="none",
        help="tag for loading full model",
    )
    if pretrain:
        parser.add_argument(
            "--convE-stag",
        ...
        parser.add_argument(
            "--seed-to-stag",
            action="store_true",
            help="add seed to tag for saving convE-layers",
        )
    if main:
        parser.add_argument(
            "--test",
            action="store_false",
            dest="train",
            help="evaluate previously saved model",
        )
    parser.add_argument(
        "--data-dir",
        type=str,
        default="{}/datasets".format(store),
        dest="d_dir",
...
def add_eval_options(
    ...
    **kwargs
    ...
    if main:
        eval_params.add_argument(
            "--pdf", action="store_true", help="generate pdf with results"
        )
    eval_params.add_argument(
    ...
    eval_params.add_argument(
        "--results-dict",
        action="store_true",
        help="output dict with results after each task",
    )
    if not comparison:
        eval_params.add_argument(
            "--loss-log",
    ...
    eval_params.add_argument(
        ...
        type=int,
        metavar="N",
        help="reference context index for drift similarity metrics",
    )
    eval_params.add_argument(
        "--drift-repr-samples",
        type=int,
        default=32,
        metavar="N",
        help="max #samples per context used for representation similarity (default: %(default)d)",
    )
    ...
    if (not no_boundaries) and (not pretrain) and (not compare_replay):
        eval_params.add_argument(
            "--sample-n", type=int, default=64, help="# images to show"
        )
        eval_params.add_argument(
            "--no-samples", action="store_true", help="don't plot generated images"
        )
    return parser
...
def add_problem_options(parser, pretrain=False, no_boundaries=False, **kwargs):
    ...
    problem_params.add_argument(
        "--experiment",
        type=str,
        default="CIFAR10" if pretrain else "splitMNIST",
        choices=(
            ["CIFAR10", "CIFAR100", "MNIST", "MNIST32"] if pretrain else cl_protocols
        ),
    )
    if no_boundaries:
        problem_params.add_argument(
    ...
    if not pretrain:
        ...
        problem_params.add_argument(
            "--contexts", type=int, metavar="N", help="number of contexts"
        )
        problem_params.add_argument(
            "--iters", type=int, help="# iterations (mini-batches) per context"
        )
        problem_params.add_argument(
            "--batch", type=int, help="mini batch size (# observations per iteration)"
        )
    if pretrain:
        problem_params.add_argument(
            "--augment",
...
def add_model_options(parser, pretrain=False, compare_replay=False, **kwargs):
    ...
    model.add_argument(
        "--reducedResNet",
        action="store_true",
        help="select defaults for 'Reduced ResNet-18' (e.g., as in Hess et al, 2023)",
    )
    # -convolutional layers
    model.add_argument(
        "--conv-type", type=str, default="standard", choices=["standard", "resNet"]
    )
    model.add_argument(
        "--n-blocks",
        type=int,
    ...
    model.add_argument(
        "--conv-nl", type=str, default="relu", choices=["relu", "leakyrelu"]
    )
    model.add_argument(
        "--global-pooling",
        action="store_true",
    ...
    model.add_argument(
        "--fc-units", type=int, metavar="N", help="# of units in hidden fc-layers"
    )
    model.add_argument(
    ...
    model.add_argument(
        "--fc-nl", type=str, default="relu", choices=["relu", "leakyrelu", "none"]
    )
    if (not pretrain) and (not compare_replay):
        model.add_argument(
            ...
            type=int,
            default=100,
            help="size of latent representation (if used, def=100)",
        )
    if not pretrain:
        model.add_argument(
            "--singlehead",
...
def add_train_options(
    ...
    **kwargs
    ...
    if pretrain:
        ...
        iter_epochs.add_argument(
            "--epochs",
            type=int,
            default=10,
            metavar="N",
            help="# epochs (default: %(default)d)",
        )
        iter_epochs.add_argument(
            "--iters", type=int, metavar="N", help='# iterations (replaces "--epochs")'
        )
    ...
    train_params.add_argument(
        ...
        type=int,
        default=0,
        help="num workers for DataLoader (default: %(default)d)",
    )
    pin_group = train_params.add_mutually_exclusive_group(required=False)
    pin_group.add_argument(
        "--pin-memory",
        action="store_true",
        dest="pin_memory",
        help="force pin_memory=True for DataLoader",
    )
    pin_group.add_argument(
        "--no-pin-memory",
        action="store_false",
        dest="pin_memory",
        help="force pin_memory=False for DataLoader",
    )
    train_params.set_defaults(pin_memory=None)
    train_params.add_argument(
        "--persistent-workers",
        action="store_true",
    ...
    train_params.add_argument(
        ...
        type=int,
        metavar="N",
        help="DataLoader prefetch factor (requires --num-workers > 0)",
    )
    train_params.add_argument(
        "--non-blocking",
        action="store_true",
        help="use non-blocking host->device transfers where supported",
    )
    if not pretrain:
        train_params.add_argument(
            "--optimizer",
    ...
    train_params.add_argument(
        "--pre-convE", action="store_true", help="use pretrained convE-layers"
    )
    train_params.add_argument(
        "--convE-ltag",
        type=str,
    ...
    train_params.add_argument(
        "--seed-to-ltag",
        action="store_true",
        help="add seed to tag when loading convE-layers",
    )
    train_params.add_argument(
    ...
    if (not pretrain) and (not no_boundaries):
        train_params.add_argument(
            ...
            dest="neg_samples",
            help="for Class-IL: which classes to set to 'active'?",
        )
    ...
    **kwargs
    ...
    if main and (not no_boundaries):
        ...
        baseline_options.add_argument(
            "--joint", action="store_true", help="train once on data of all contexts"
        )
        baseline_options.add_argument(
            "--cummulative",
            action="store_true",
    ...
    xdg_message = (
        "use 'Context-dependent Gating' (Masse et al, 2018)"
        if main
        else "combine all methods with XdG"
    )
    context_spec.add_argument("--xdg", action="store_true", help=xdg_message)
    context_spec.add_argument(
        "--gating-prop",
        type=float,
        metavar="PROP",
    ...
    if main:
        context_spec.add_argument(
            "--separate-networks",
            action="store_true",
            help="train separate network per context",
        )
    if compare_all:
        context_spec.add_argument(
            "--fc-units-sep",
    ...
    if not compare_replay:
        ...
        if main and no_boundaries:
            ...
            param_reg.add_argument(
                "--si",
                action="store_true",
                help="select defaults for 'SI' (Zenke et al, 2017)",
            )
            param_reg.add_argument(
                "--weight-penalty",
                action="store_true",
        ...
        if main and not no_boundaries:
            ...
            param_reg.add_argument(
                "--importance-weighting", type=str, choices=["fisher", "si", "owm"]
            )
        if not no_boundaries:
            param_reg.add_argument(
                "--fisher-n",
                type=int,
                help="-> Fisher: sample size estimating Fisher Information",
            )
            param_reg.add_argument(
                "--fisher-batch",
                type=int,
            ...
            param_reg.add_argument(
                "--fisher-init",
                action="store_true",
                help="-> Fisher: start with prior (as in NCL)",
            )
            param_reg.add_argument(
                "--fisher-prior",
                type=float,
        ...
        if compare_all:
            param_reg.add_argument(
                "--c", type=float, dest="si_c", help="-> SI: regularisation strength"
            )

    ## Functional regularization
    func_reg = parser.add_argument_group("Functional Regularization")
    if main:
        func_reg.add_argument(
            "--lwf",
            action="store_true",
            help="select defaults for 'LwF' (Li & Hoiem, 2017)",
        )
        func_reg.add_argument(
            "--distill",
            action="store_true",
            help="use distillation-loss for the replayed data",
        )
    if not compare_replay:
        func_reg.add_argument(
            "--temp",
    ...
    if main and not no_boundaries:
        func_reg.add_argument(
            "--fromp", action="store_true", help="use 'FROMP' (Pan et al, 2020)"
        )
    if (not compare_hyper) and not no_boundaries:
        func_reg.add_argument(
            "--tau", type=float, help="-> FROMP: regularization strength"
        )
    if compare_replay:
        func_reg.add_argument(
            "--tau-per-budget",
    ...
    if not compare_replay:
        buffer.add_argument(
            "--budget",
            type=int,
            help="how many samples can be stored{}".format(
    ...
    if main and not no_boundaries:
        buffer.add_argument(
            "--sample-selection", type=str, choices=["random", "herding", "fromp"]
        )
        buffer.add_argument(
            "--add-buffer",
            action="store_true",
    ...
    if main:
        replay_choices = (
            ["none", "current", "buffer"]
            if no_boundaries
            else ["none", "all", "generative", "current", "buffer"]
        )
        ...
        replay_params.add_argument(
            ...
            type=str,
            default="normal",
            choices=["normal", "inequality", "both"],
        )
    ...
    if (not compare_replay) and (not no_boundaries):
        ...
        if not compare_hyper:
            replay_params.add_argument(
                "--g-z-dim",
                type=int,
                help="size latent space generator (def: as classifier)",
            )
            replay_params.add_argument(
                "--g-fc-lay",
                type=int,
                help="[fc_layers] in generator (def: as classifier)",
            )
            replay_params.add_argument(
                "--g-fc-uni",
                type=int,
                help="[fc_units] in generator (def: as classifier)",
            )
            replay_params.add_argument(
                "--g-iters",
                type=int,
                help="# batches to train generator (def: as classifier)",
            )
            replay_params.add_argument(
                "--lr-gen",
                type=float,
                help="learning rate generator (def: as classifier)",
            )
        ...
        if main:
            ...
            replay_params.add_argument(
                "--prior", type=str, default="standard", choices=["standard", "GMM"]
            )
            replay_params.add_argument(
                "--per-class",
                action="store_true",
        ...
        if main:
            replay_params.add_argument(
                "--dg-gates",
                action="store_true",
                help="use context-specific gates in decoder",
            )
        replay_params.add_argument(
            "--dg-type",
            type=str,
        ...
        if not compare_hyper:
            replay_params.add_argument(
                "--dg-prop", type=float, help="decoder-gates: masking-prop"
            )
    if main:
        replay_params.add_argument(
            "--hidden",
    ...
    if not compare_replay:
        ...
        if main:
            ...
            templ_cl.add_argument(
                "--prototypes",
                action="store_true",
                help="classify using nearest-exemplar-mean rule",
            )
            templ_cl.add_argument(
                "--gen-classifier",
                action="store_true",
        ...
        if compare_all:
            ...
            templ_cl.add_argument(
                ...
                type=int,
                metavar="N",
                help="# fc-layers with generative classifier",
            )
            templ_cl.add_argument(
                "--z-dim-gc",
                type=int,
                metavar="N",
                help="size latent space generative classifier",
            )
    return parser
